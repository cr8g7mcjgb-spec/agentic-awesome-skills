...
function scanObjects(latin, buf) {
  ...
  while ((m = re.exec(latin)) !== null) {
    ...
    const body = latin.slice(bodyAt, endAt < 0 ? Math.min(bodyAt + 2e5, latin.length) : endAt);
...
async function fontTable(objects, fontObjNum) {
  ...
  if (/\/Type0\b/.test(font.dict) || /\/Identity-[HV]/.test(font.dict)) {
    return { map: /* @__PURE__ */ new Map(), codeBytes: 2 };
  }
  return null;
}
function scanContent(text, onShow, onBreak, onFont) {
  ...
  while (i < text.length) {
    ...
    if (c === "[") {
      operands.push({ kind: "arrayStart" });
      i++;
      continue;
    }
    if (c === "]") {
      operands.push({ kind: "arrayEnd" });
      i++;
      continue;
    }
    if (c === "/") {
      let j2 = i + 1;
      while (j2 < text.length && !/[\s/<>[\]()]/.test(text[j2])) j2++;
      operands.push({ kind: "name", value: text.slice(i + 1, j2) });
      i = j2;
      continue;
    }
    if (/[-+.\d]/.test(c)) {
      let j2 = i;
      while (j2 < text.length && /[-+.\d]/.test(text[j2])) j2++;
      operands.push({ kind: "num", value: text.slice(i, j2) });
      i = j2;
      continue;
    }
    let j = i;
...
function decodePlain(bytes) {
  let ascii = true;
  for (const b of bytes) if (b > 126 || b < 32 && b !== 9 && b !== 10 && b !== 13) {
    ascii = false;
    break;
  }
  ...
  for (const [, page] of pages) {
    ...
    scanContent(
      ...
      (name) => {
        ...
        table = num === void 0 ? null : tables.get(num) ?? null;
      }
    );
