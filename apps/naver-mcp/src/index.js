...
const TOOLS = [
  ...
  },
  {
    name: "read_article",
    description:
      "URL 하나로 본문을 읽는다. 네이버 블로그/뉴스/공개 카페, 티스토리는 전용 추출기로, 그 밖의 모든 사이트(정부·공공기관, 연구소, 언론사 등)는 범용 추출기로 처리한다. 검색 결과에서 얻은 링크는 종류를 가리지 말고 이 도구에 넣어라. 응답 맨 위에 항상 출처 링크가 포함되고, 페이지에 첨부파일이 있으면 그 주소도 함께 돌려준다 - 그 주소를 read_file 에 넣으면 PDF/HWPX 본문을 읽을 수 있다.",
    inputSchema: {
      type: "object",
      properties: {
        url: {
    ...
    case "initialize": {
      ...
      return rpcResult(id, {
        ...
        instructions:
          ...
          "공공기관 자료는 검색 → read_article 로 공고 페이지를 열고 → 거기 딸려 나오는 " +
          "첨부파일 주소를 read_file 에 넣는 순서로 접근하라. " +
